...
void init_7seg(void) {
    ...
    ANSELF = ANSELF & ~(1 << 8); // inhibit analog functions on B12 and B13
...
void main() {
    ...
    //SPI 
    SPI1CON |= (1 << 15 | 1 << 8 | 1 << 5);
    SPI1CON = SPI1CON &~(1 << 31 | 1 << 23 | 1 << 16 | 1 << 10 | 1 << 11);
    SPI1CON2 = SPI1CON2 &~(1 << 7);
    SPI1BRG = 2;
    RPF2R = 0x8;
    SDI1R = 0xF;
    TRISF = TRISF &~(1 << 8 | 1 << 6);
    ...
    while (1) {
        ...
        LCD_Write_HEX(toto);
          * */
        (n<16)?LCD_Set_Cursor_Pos(0, n):LCD_Set_Cursor_Pos(1, n-16);
        LCD_Write_Char(toto);
        n++; 
        /*
        LCD_Set_Cursor_Pos(2, 12);
        LCD_Write_String("0x");
        LCD_Write_HEX(cpt);
        for (int i = 0; i < 100; i++)
            DelayUs(10000);
